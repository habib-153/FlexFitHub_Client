...
const navLinks = [
  ...
  { name: "Checkout", path: "/products/checkout" },
    ...
    <ul className="my-2 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      {navLinks.map((link) => (
        <li key={link.name} className="p-1 font-medium">
          <NavLink to={link.path} className="flex items-center hover:text-blue-500 transition-colors">
            {link.name}
          </NavLink>
        </li>
      ))}
...
const Header = () => {
  ...
  return (
    <div className="bg-[#1a1c1cfd]">
      <Navbar className="mx-auto max-w-[1200px] bg-transparent border-0 px-0  w-full py-3" placeholder="" onPointerEnterCapture={() => {}} onPointerLeaveCapture={() => {}}>
        <div className="flex items-center justify-between text-[#FFFFFF]">
          <NavLink to="/" className="mr-4 cursor-pointer py-1.5 font-bold text-xl flex items-center gap-2">
          <p>FlexFitHub</p>
